...
// Удаляем фиксированную стоимость перерождения - теперь она динамическая
...
const shopCatalog = [
  ...
  // Новые плюшки:
  {
    ...
    desc: "+100 пассивной энергии в секунду.",
    baseCost: 15000,
    currency: "credits",
    passiveBonus: 100,
  },
  ...
  },
  {
    ...
    desc: "Умножение всей энергии на 1.5 при покупке.",
    baseCost: 12000,
    currency: "energy",
    immediateEffect: "energyMultiplier",
  },
  {
    id: "creditFountain",
    title: "Credit Fountain",
    desc: "Каждые 10 секунд даёт 50 кредитов.",
    baseCost: 12000,
    currency: "credits",
    timedReward: { currency: "credits", amount: 50, interval: 10000 },
  },
  {
    id: "dataHarvester",
    title: "Data Harvester",
    desc: "Пассивно собирает 1 данные каждые 5 секунд.",
    baseCost: 18000,
    currency: "data",
    timedReward: { currency: "data", amount: 1, interval: 5000 },
  },
  {
    id: "nexusCore",
    title: "Nexus Core",
    desc: "Мощный реактор, дающий +500 пассивной энергии.",
    baseCost: 50000,
    currency: "credits",
    passiveBonus: 500,
  },
  {
    id: "timeDilation",
    title: "Time Dilation",
    desc: "Все эффекты работают в 2 раза быстрее.",
    baseCost: 25000,
    currency: "data",
  ...
  },
  {
    ...
    desc: "Удваивает ценность каждого клика.",
    baseCost: 22000,
    currency: "energy",
    clickMultiplier: 2,
  },
  {
    ...
    desc: "Автоматически добавляет +1 к очкам перерождения.",
    baseCost: 75000,
    currency: "data",
    autoPrestigeBonus: 1,
  },
...
const prestigeCatalog = [
  ...
  {
    id: "sparkAmplifier",
    title: "Spark Amplifier",
    desc: "Удваивает все искры от перерождения.",
    cost: 5,
    effects: { prestigeMultiplier: 2 },
  },
  {
    id: "energyBeacon",
    title: "Energy Beacon",
    desc: "Начальная энергия: +1000.",
    cost: 3,
    effects: { initialEnergy: 1000 },
  },
  {
    id: "creditNexus",
    title: "Credit Nexus",
    desc: "Автоматические +5 кредитов в секунду.",
    cost: 4,
    effects: { autoCredits: 5 },
  },
  {
    id: "dataMatrix",
    title: "Data Matrix",
    desc: "Все данные удваиваются.",
    cost: 6,
    effects: { dataMultiplier: 2 },
  },
];
...
const achievements = [
  ...
  },
  {
    ...
    description: "Накопить 100,000 энергии.",
    reward: { credits: 500, data: 15 },
    condition: (s) => s.energy >= 100000,
  },
  {
    id: "spender",
    title: "Щедрый Кнот",
    description: "Потратить 10,000 энергии.",
    reward: { passiveRate: 50 },
    condition: (s) => totalSpent("energy") >= 10000,
  },
  {
    id: "shopLover",
    title: "Любитель лавки",
    description: "Купить 25 разных предметов в магазине.",
    reward: { globalMultiplier: 1.1 },
    condition: (s) => {
      return Object.values(s.inventory).filter(inv => 
        typeof inv === "object" && inv.count && inv.count > 0
      ).reduce((total, inv) => total + inv.count, 0) >= 25;
    },
  },
  {
    id: "speedDemon",
    title: "Демон скорости",
    description: "Достичь 15 ударов в секунду.",
    reward: { energy: 1000, credits: 200 },
    condition: (s) => {
      // Проверяем количество кликов за последние 3 секунды
      const now = Date.now();
      const recentClicks = clickTimestamps.filter(ts => now - ts <= 3000);
      return recentClicks.length >= 15;
    },
  },
  {
    id: "timeWaster",
    title: "Поглотитель времени",
    description: "Играть 30 минут подряд.",
    reward: { globalMultiplier: 1.08 },
    condition: (s) => {
      const sessionTime = Date.now() - sessionStart;
      return sessionTime >= 30 * 60 * 1000; // 30 минут в миллисекундах
    },
  },
  {
    id: "dataCollector",
    title: "Коллекционер данных",
    description: "Собрать 200 данных.",
    reward: { credits: 300 },
    condition: (s) => s.data >= 200,
  },
  {
    id: "prestigeHunter",
    title: "Охотник за искрами",
    description: "Совершить 3 перерождения.",
    reward: { prestigePoints: 3 },
    condition: (s) => s.prestige >= 3,
  },
  {
    id: "millionaire",
    title: "Миллионер Кнот",
    description: "Накопить 1,000,000 кредитов.",
    reward: { globalMultiplier: 1.15, prestigePoints: 2 },
    condition: (s) => s.credits >= 1000000,
  },
  {
    id: "clickMachine",
    title: "Клик-машина",
    description: "Сделать 1000 кликов.",
    reward: { energy: 500, data: 10 },
...
// Новая функция для расчёта стоимости следующего перерождения
function getPrestigeCost() {
  const baseCosts = [5000, 10000, 50000, 250000, 1000000, 5000000, 25000000, 100000000, 500000000, 2500000000];
  const prestigeCount = state.prestige;
  if (prestigeCount < baseCosts.length) {
    return baseCosts[prestigeCount];
  } else {
    // Для перерождений выше 9 используем экспоненциальный рост
    return Math.floor(10000000000 * Math.pow(10, prestigeCount - 9));
  }
}
...
function performPrestige() {
  ...
  if (state.credits < cost) {
    addToast("Перерождение", `Нужно минимум ${formatNumber(cost)} кредитов.`);
    return;
  }
  
  let gain = Math.floor(state.credits / cost);
  // Применяем множитель искр от prestigeBooster
  if (state.prestigeInventory?.sparkAmplifier) {
    gain *= 2; // удваиваем искры
  }
  ...
  // Применяем начальные бонусы от перерождения
  if (state.prestigeInventory?.energyBeacon) {
    state.energy += 1000;
  }
  if (state.prestigeInventory?.creditNexus) {
    state.passiveRate += 5;
  }
...
function purchase(item) {
  ...
  if (item.immediateEffect === "energyMultiplier") {
    state.energy = Math.floor(state.energy * 1.5);
    addToast("Energy Cascade", "Энергия умножена на 1.5!");
  }
...
function updatePPSDisplay() {
  ...
  // Правильный расчет PPS: количество кликов в текущем окне
  const rate = clickTimestamps.length / (PPS_WINDOW / 1000);
...
function applyPrestigeEffect(item) {
  ...
  if (item.effects.prestigeMultiplier) {
    // Эффект будет применен в performPrestige
  }
  if (item.effects.initialEnergy) {
    // Эффект будет применен в performPrestige
  }
  if (item.effects.autoCredits) {
    // Эффект будет применен в performPrestige
  }
  if (item.effects.dataMultiplier) {
    // Эффект будет применен при получении данных
  }
}
...
function renderPrestigeStats() {
  ...
  selectors.prestigeCost.textContent = gain
    ? `готово: ${gain} искр`
    : `нужно ${formatNumber(cost)} кредитов`;
...
function handleClick() {
  ...
  if (dataBonus) {
    let finalDataBonus = dataBonus * activeMultiplier();
    if (state.prestigeInventory?.dataMatrix) {
      finalDataBonus *= 2;
    }
    state.data += finalDataBonus;
  }
  triggerBurstRewards();
  renderResources();
  checkAchievements();
